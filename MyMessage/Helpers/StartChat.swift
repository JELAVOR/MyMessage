...
func removeMemberWhoHasRecent(snapshot: QuerySnapshot, memberIds:[String]) -> [String] {
    ...
    for recentData in snapshot.documents {
        ...
        if let currentUserIds = currentRecent[KSENDERID] {
            if memberIdsToCreateRecent.contains(currentUserIds as! String) {
                memberIdsToCreateRecent.remove(at: memberIdsToCreateRecent.firstIndex(of: currentUserIds as! String)!)
            }
        }
    }
